import torch
import torch.nn as nnContextnet
    ...
    def forward(self, x, timestep=0.5, scale_list=[8, 4, 2, 1], training=False, fastmode=True, ensemble=False):
        ...
        for i in range(4):
            if flow is None:
                ...
                wf0 = warp(f0, flow[:, :2], self.gpu_n)
                wf1 = warp(f1, flow[:, 2:4], self.gpu_n)
            ...
            warped_img0 = warp(img0, flow[:, :2], self.gpu_n)
            warped_img1 = warp(img1, flow[:, 2:4], self.gpu_n)
